...
def register():
    ...
    if form.validate_on_submit():
        ...
        #!use `current_app.current_app_context()' only when working outside a request context, such as background jobs or scripts. For Flask routes, it's not needed, as Flask automatically manages the context.
        db.session.add(user)
        db.session.commit()
...
def reset_token(token):
        ...
        if user is None:
            flash("Invalid Token or Token is expired", "warning")
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(f"Password has been reset successfully", "success")
            return redirect(url_for('users.login'))
